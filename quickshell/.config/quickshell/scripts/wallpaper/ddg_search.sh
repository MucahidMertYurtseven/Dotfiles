...
python3 -u "$SCRIPT_DIR/get_ddg_links.py" "$QUERY" | while IFS='|' read -r thumb_url full_url; do
    ...
    if [[ ! "$target_type" =~ "image/" ]]; then
        echo "Skip: dead URL ($target_type)" >> "$LOG_FILE"
        continue
    fi
    ...
    state=$(cat "$CONTROL_FILE" 2>/dev/null | tr -d '[:space:]')
    if [[ "$state" == "stop" ]]; then
        echo "Stop signal received during download." >> "$LOG_FILE"
        rm -f "$tmppath"
        exit 0
    fi
    ...
    if [ -s "$tmppath" ]; then
        ...
        if [[ ! "$actual_mime" =~ ^image/ ]]; then
            echo "ERROR: Not an image ($actual_mime)." >> "$LOG_FILE"
            rm -f "$tmppath"
        else
        ...
        echo "ERROR: Empty download for $thumb_url" >> "$LOG_FILE"
        rm -f "$tmppath"
    fi
done
