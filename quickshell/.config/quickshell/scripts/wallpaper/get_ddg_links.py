...
def main():
    ...
    for i in range(3):
        try:
            ...
            if match:
                vqd = match.group(1)
                log(f"Found VQD token: {vqd}")
                break
            else:
                log(f"Attempt {i+1}: No VQD found")
        except Exception as e:
            log(f"Attempt {i+1} error: {str(e)}")
            time.sleep(1)

    if not vqd:
        log("Failed to get VQD token. Exiting.")
        return
    ...
    for page in range(5):
        ...
        try:
            ...
            log(f"Error page {page + 1}: {str(e)}")
            break

    log(f"=== SEARCH COMPLETE. FHD links: {links_found} ===")
